...
interface WaveformVisualizerProps { isActive?: boolean; barCount?: number; }
    ...
    <div className="flex flex-col items-center gap-2">
      <div ref={barsRef} className="flex items-end gap-[3px] h-16 w-60 will-change-transform">
        {Array.from({ length: barCount }).map((_, i) => (
          <div key={i} className="w-[3px] rounded-full bg-brand opacity-70" style={{ height: 24 }} />
        ))}
      </div>
      <div className="flex items-center gap-1.5">
        <span className="w-2 h-2 rounded-full bg-brand animate-pulse" />
        <span className="font-mono text-[10px] text-ink-500 uppercase tracking-wider">Agente in ascolto...</span>
      </div>
    </div>
  );
};
