...
Deno.serve(async (req) => {
  ...
  const FN = "check-documenti-scadenze";
  ...
  try {
    ...
    if (!docs || docs.length === 0) return jsonOk({ message: "Nessun documento in scadenza" }, rid);
    ...
    for (const doc of docs) {
      ...
      if (diffDays <= 0 && !doc.alert_scaduto) {
        alertType = "scaduto"; shouldUpdate = true;
        ...
        alertType = "7g"; shouldUpdate = true;
    ...
    if (RESEND_API_KEY && alerts.length > 0) {
      ...
      for (const [companyId, data] of Object.entries(byCompany)) {
        const { data: profiles } = await adminClient.from("profiles").select("email").eq("company_id", companyId);
        ...
        const html = `<div style="font-family:sans-serif;max-width:600px;margin:auto">
          ...
          <table style="width:100%;border-collapse:collapse;margin:20px 0">
            <thead><tr style="background:#f1f5f9">
              <th style="padding:8px;text-align:left">Persona</th><th style="padding:8px;text-align:left">Tipo</th>
              <th style="padding:8px;text-align:left">Documento</th><th style="padding:8px;text-align:left">Scadenza</th>
              <th style="padding:8px;text-align:left">Stato</th>
            </tr></thead><tbody>${alertRows}</tbody>
          </table>
          <p style="color:#666;font-size:12px">Accedi a edilizia.io per gestire i documenti.</p></div>`;
        ...
        try {
          ...
          log("warn", "Resend scadenze email failed", { request_id: rid, fn: FN, company_id: companyId, error: e instanceof Error ? e.message : "unknown" });
        }
      }
    }

    log("info", "Documenti scadenze check done", { request_id: rid, fn: FN, alerts_sent: alerts.length });
    return jsonOk({ alerts_sent: alerts.length, details: alerts }, rid);
  } catch (err) {
