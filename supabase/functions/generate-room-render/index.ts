...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Extract image from response - could be base64 in content or a URL
    let resultBase64 = "";
    let resultUrl = "";

    if (typeof parts === "string") {
      // Text response - check if it contains a URL or base64
      if (parts.startsWith("data:image")) {
        resultBase64 = parts.split(",")[1] || "";
      } else {
        // Might be a text-only response, return error
