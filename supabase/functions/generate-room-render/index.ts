...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Resolve company_id — from session if provided, else from user profile
    let companyId: string | null = null;
    if (session_id) {
      const { data: session } = await supa.from("render_stanza_sessions").select("user_id").eq("id", session_id).single();
      if (session) {
        // Verify ownership: session must belong to calling user
    ...
    log("info", "Generating room render", { request_id: rid, fn: FN, session_id, prompt_length: prompt?.length });
    ...
    // Check images field first (some models)
    ...
    if (resultBase64 && session_id) {
      ...
      const { error: upErr } = await supa.storage.from("stanza-results").upload(path, bArr, { contentType: "image/jpeg", upsert: true });
      if (!upErr) {
        const { data: urlData } = supa.storage.from("stanza-results").getPublicUrl(path);
        resultUrl = urlData.publicUrl;
        await supa.from("render_stanza_sessions")
          .update({ result_image_url: resultUrl, result_image_path: path, status: "completed" })
          .eq("id", session_id);
      }
    } else if (resultUrl && session_id) {
      // Remote URL — still persist to session
      await supa.from("render_stanza_sessions")
        .update({ result_image_url: resultUrl, status: "completed" })
        .eq("id", session_id);
    }

    // Deduct render credit AFTER successful generation and upload
    if (companyId && (resultBase64 || resultUrl)) {
      await supa.rpc("deduct_render_credit", { _company_id: companyId });
    }

    log("info", "Room render generated", { request_id: rid, fn: FN, has_result: !!(resultBase64 || resultUrl) });
