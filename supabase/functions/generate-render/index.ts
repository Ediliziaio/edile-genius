...
function buildPromptFromConfig(session: any): { systemPrompt: string; userPrompt: string; negativePrompt: string; promptVersion: string; blocks: Record<string, string> } {
  ...
  const oldMatDesc: Record<string, string> = { legno_vecchio: "aged wood with visible weathering", legno_verniciato: "painted wood with possible peeling", alluminio_anodizzato: "old anodized aluminum", alluminio_verniciato: "painted aluminum with fading", pvc_bianco: "white PVC possibly yellowed", pvc_colorato: "colored PVC with fading", ferro: "old iron frame with possible rust", acciaio: "steel frame with paint wear", sconosciuto: "existing frame" };
  ...
  const profiloSize: Record<string, string> = { "70mm": "70mm residential profile with 3 internal chambers", "82mm": "82mm premium profile with 5 internal chambers", "92mm": "92mm Passivhaus-grade profile with 7 internal chambers" };
  const profiloForma: Record<string, string> = { squadrato: "squared/angular edges", arrotondato: "softly rounded edges", europeo: "classic European profile with slight bevel" };
  ...
  blocks.G = oscurante.tipo && oscurante.tipo !== "nessuno" ? `[BLOCK G – SHUTTERS/BLINDS]\n${oscurante.prompt_fragment || oscurante.tipo}` : `[BLOCK G – SHUTTERS/BLINDS]\nNo external shutters or blinds.`;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 120s timeout — no retry (expensive, credits already checked)
