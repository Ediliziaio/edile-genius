...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Auth: either cron (service role) or user JWT
    const authHeader = req.headers.get("Authorization");
    let companyId: string | null = null;

    // Check if specific company_id was passed (for cron/service calls)
    let body: any = {};
    try {
      body = await req.json();
    ...
    // If no specific company, process ALL companies with enabled automation
    const companyIds: string[] = [];

    if (companyId) {
      companyIds.push(companyId);
    } else {
      // Cron mode: find all companies with followup_agent enabled
    ...
    // Update last_run_at for all processed companies
    if (companyIds.length > 0) {
...
async function processCompany(sb: any, companyId: string, rid: string): Promise<number> {
  ...
  if (credits?.calls_blocked || (Number(credits?.balance_eur) || 0) < 0.50) {
    log("info", "Skipping company — insufficient credits", { request_id: rid, company_id: companyId });
    return 0;
  }
  ...
  for (const contact of dormantContacts) {
    try {
      // Find an agent to use: prefer assigned_agent, then any active outbound agent
      ...
      if (!agentData?.el_agent_id || !agentData?.el_phone_number_id) {
        ...
        break; // No point continuing for this company
      }
