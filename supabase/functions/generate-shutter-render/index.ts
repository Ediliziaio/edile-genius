...
Deno.serve(async (req) => {
  ...
  const FN = "generate-shutter-render";
  ...
  try {
    ...
    if (session_id) {
      const { data: sess } = await supabase.from("render_persiane_sessions").select("company_id").eq("id", session_id).single() as any;
    ...
    log("info", "Generating shutter render", { request_id: rid, fn: FN, session_id, prompt_length: prompt?.length });
    ...
    if (resultBase64 && session_id) {
      ...
      const { error: upErr } = await supa.storage.from("persiane-results").upload(path, bArr, { contentType: "image/jpeg", upsert: true });
      if (!upErr) {
        const { data: urlData } = supa.storage.from("persiane-results").getPublicUrl(path);
        resultUrl = urlData.publicUrl;
        await supa.from("render_persiane_sessions")
          .update({ result_image_url: resultUrl, status: "completed" } as any)
          .eq("id", session_id);
    ...
    log("info", "Shutter render generated", { request_id: rid, fn: FN, has_result: !!(resultBase64 || resultUrl) });
